...
function [A B C D E F G] = ABCs_logicals(num, vec1, vec2, str1, str2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% and simply checks to see if they are equal. It is convenient because it 
% will work with inputs of differing lengths, unlike the double equals 
...
% one string matches the other. For example, you might decide to code your
% own version of sort() and need to know if an inputted string says
% 'ascend' or 'descend'. If you try to compare these with a double-equals
% sign, then it will error because the strings are not the same length. So
% there is a function designed specifically to compare strings: strcmp().
% There is also the function strcmpi(), which also compares strings but
% ignores case. Use this knowledge (along with everything else in this
% file) to find if the inputs str1 and str2 are the same but with differing
% case (G).
G = strcmpi(str1,str2)
end